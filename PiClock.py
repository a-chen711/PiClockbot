...
#=====================================
def time_update(curr_hour): 
    midnight = False
    hour_list = []
    for i in range(4):
        if midnight:
            next_hour = str(int(next_hour) + 1)
        elif int(curr_hour) + i + 1 >= 24:
            next_hour = "00"
            midnight = True
        else:
            next_hour = str(int(curr_hour) + i + 1)
...
def wifi_connected(): #test internet connection
...
try:
    ...
    weather_img = []
    for i in range(18):
        weather_img.append(Image.open("./resized/" + weather_list[i] + "_v2.png"))
    ...
    while True:
        ...
        count +=1
        time.sleep(0.01) #this might need to change
        # disp.display()
    disp.module_exit()
except IOError as e:
    logging.info(e)    
